$(function () {
  ...
  (function init() {
    ...
    function generate() {
      ...
      let shouldEnhanceFaultTolerance = $("#faultToleranceCheckBox").prop(
        "checked"
      );
      ...
      let getIterateLines = (
        ...
        if (inner === undefined || inner === null) {
          showInfo(
            `WARNING : the property named &nbsp <b>'${key}'</b> &nbsp is an EMPTY array ! parse process is failed !`
          );
          return {
        ...
        while (count) {
          fromJsonLines.unshift(
            `${makeBlank(count * 2)}v.forEach((v) {\n${makeBlank(
              count * 3
            )}final arr${count} = ${genericStringGenerator(
              innerClass,
              total - count
            ).slice(4)}[];`
          );
        ...
        let typeCheck = shouldEnhanceFaultTolerance
          ? ` && (json[${jsonKey}] is List)`
          : "";
        fromJsonLines.unshift(
          `${makeBlank(
            count * 2
            ...
            count * 2
          )}final arr0 = ${genericStringGenerator(innerClass, total).slice(
            4
          )}[];`
        );
        fromJsonLines.push(
        ...
        for (let key in jsonObj) {
          if (jsonObj.hasOwnProperty(key)) {
            ...
            if (shouldUsingJsonKey) {
              jsonKey = `${
                isJsonKeyPrivate ? "_" : ""
              }jsonKey${className}${uppercaseFirst(legalKey)}`;
            }
            ...
            if (element === null) {
              //!显示错误信息
              showInfo(
                `WARNING : the Property named '${key}' is null,which will be treated as String type`
              );
              element = "";
            }
            if (typeof element === "object") {
              ...
              if (Array.isArray(element)) {
                ...
                let { fromJsonLinesJoined, toJsonLinesJoined } =
                  getIterateLines(
                    element,
                    subClassName,
                    key,
              ...
              if (typeof element === "boolean") {
                ...
                if (typeof element === "string") {
                  ...
                  if (Number.isInteger(element)) {
                    toType = shouldEnhanceFaultTolerance
                      ? `int.tryParse(json[${jsonKey}]?.toString() ?? '')`
                      : `json[${jsonKey}]?.toInt()`;
                    type = "int";
                  } else {
                    toType = shouldEnhanceFaultTolerance
                      ? `double.tryParse(json[${jsonKey}]?.toString() ?? '')`
                      : `json[${jsonKey}]?.toDouble()`;
                    type = "double";
                  }
                }
              }
              propsLines.push(
                `  ${type}${shouldNullSafe ? "?" : ""} ${legalKey};\n`
              );
...
*/\n\n${objToDart(jsonObj, rootClass, "")}`;
    ...
    function textFieldBinding(tfID, defaultValue) {
      ...
      if (
        (strFromCookie === undefined || strFromCookie.length === 0) &&
        defaultValue
      ) {
    ...
    $("#jsonKeyPrivateCheckBox").prop(
      "disabled",
      !$("#usingJsonKeyCheckBox").prop("checked")
    );
